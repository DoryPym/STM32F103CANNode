...
/// 将libcanard ID值转换为bxCAN TX ID寄存器格式。
...
/// 将bxCAN TX / RX（原文如此，两个RX / TX都支持）ID寄存器值转换为libcanard ID格式。
...
static bool waitMSRINAKBitStateChange(volatile const CanardSTM32CANType* const bxcan, const bool target_state)
{
     ...
     * 正常运行的总线在每次正确传输的末尾将显示11个连续的隐性位，或在总线空闲时，11个连续的隐性位包括：
     * 1位-确认定界符
     * 7位-帧结束位
     * 3位-帧间空间
     * 总计为11； 因此，实际上不必等待几个帧TX间隔。
     */
    ...
    for (uint16_t wait_ack = 0; wait_ack < TimeoutMilliseconds; wait_ack++)
    {
        ...
        //usleep(1000);           // TODO: This function may be missing on some platforms， 在某些平台上可能缺少此功能
    }
                        ...
                        const CanardSTM32IfaceMode iface_mode)
{
    ...
    // CAN1将永远处于初始化模式，在这种模式下，它完全不影响总线。
#endif

    BXCAN->IER = 0;                                             // We need no interrupts，我们不开启中断
    BXCAN->MCR &= ~CANARD_STM32_CAN_MCR_SLEEP;                  // Exit sleep mode，退出睡眠模式
    BXCAN->MCR |= CANARD_STM32_CAN_MCR_INRQ;                    // Request init，请求初始化

    if (!waitMSRINAKBitStateChange(BXCAN, true))                // Wait for synchronization，等待同步
    {
    ...
    CANARD_ASSERT(0 == BXCAN->IER);             // Making sure the iterrupts are indeed disabled，确保确实禁用了迭代

    BXCAN->MCR &= ~CANARD_STM32_CAN_MCR_INRQ;   // Leave init mode，离开初始化模式
     ...
     * 默认过滤器配置。 请注意，所有过滤器仅可通过CAN1使用！ CAN2滤波器偏移了14。
     * 我们最多始终使用14个过滤器，以简化代码并确保与所有过滤器的兼容所有的STM32系列中的MCU。
     */
    {
    ...
    // 过滤器在FIFO0和FIFO1之间交替使用，以均衡负载。这会导致接收时偶尔发生优先级倒置和帧重新排序，但对于UAVCAN来说是可以接受的，并且其他大多数协议都可以接受
    // 因为在同一CAN ID中不会进行重新排序。
    ...
    CANARD_STM32_CAN1->FA1R = 1;                                        // One filter enabled, 启用了一个过滤器
#endif

    CANARD_STM32_CAN1->FMR &= ~CANARD_STM32_CAN_FMR_FINIT;              // Leave initialization mode ，离开初始化模式
...
int16_t canardSTM32Transmit(const CanardCANFrame* const frame)
{
     ...
     * 寻找一个空的插槽，检查如果我们现在入队是否会发生优先级倒置。如果所有TX邮箱都是空闲的，并且没有待发送的邮件，我们总是可以安全地排队。
     */
    ...
    if ((BXCAN->TSR & AllTME) != AllTME)                // At least one TX mailbox is used, detailed check is needed，至少使用一个TX邮箱，需要详细检查
    {
        const bool tme[3] =
        {
        ...
        for (uint8_t i = 0; i < 3; i++)
        {
            ...
            {                                           // 该TX邮箱处于待处理状态，请检查优先级倒置
                if (!isFramePriorityHigher(frame->id, convertFrameIDRegisterToCanard(BXCAN->TxMailbox[i].TIR)))
                {
                    // There's a mailbox whose priority is higher or equal the priority of the new frame.有一个邮箱的优先级高于或等于新框架的优先级。
                    return 0;                           // Priority inversion would occur! Reject transmission.优先级反转！ 拒绝传输。
                }
            }
        }
        ...
        if (tx_mailbox == 0xFF)
        {
             ...
             * 所有TX邮箱都忙（这是极不可能的）；同时我们知道没有当前未决的更高或相同优先级的帧。因此，优先级倒置具有
             * 刚刚发生（很简单！），因为由于所有TX邮箱，我们无法排队更高优先级的帧忙着。这种情况极不可能发生，因为为了使其发生，应用程序
             * 将需要传输4（四个）或更多具有不同CAN ID的CAN帧，从高ID到低ID几乎同时出现。例如：
             * 1. 0x123 <-接收邮箱0（或其他任何邮箱）
             * 2. 0x122 <-获取邮箱2（或其他任何邮箱）
             * 3. 0x121 <-获取邮箱1（或其他任何邮箱）
             * 4. 0x120 <-内部优先级反转（仅在所有三个TX邮箱仍然繁忙时）
             *这种情况更不可能引起CAN总线的任何明显中断。尽管那样，最好在调试期间就警告开发人员，所以我们在这里触发断言失败。完全安全地删除它。
             */
    ...
    CANARD_ASSERT(tx_mailbox < 3);                      // Index check - the value must be correct here，索引检查-此处的值必须正确
     ...
     * 
     * 到此时，我们已经证明不会发生优先级倒置，并且我们还找到了一个免费的TX邮箱。 因此，现在将框架放入队列是安全的。
     * 
     */
    volatile CanardSTM32TxMailboxType* const mb = &BXCAN->TxMailbox[tx_mailbox];

    mb->TDTR = frame->data_len;                         // DLC equals data length except in CAN FD，DLC等于数据长度，CAN FD中除外
