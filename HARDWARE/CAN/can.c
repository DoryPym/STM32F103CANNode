...
void HAL_CAN_RxCpltCallback(CAN_HandleTypeDef* hcan)
{
    ...
    printf("id:%d\r\n",CAN2_Handler.pRxMsg->StdId);
    printf("ide:%d\r\n",CAN2_Handler.pRxMsg->IDE);
    printf("rtr:%d\r\n",CAN2_Handler.pRxMsg->RTR);
    printf("len:%d\r\n",CAN2_Handler.pRxMsg->DLC);
    for(i=0;i<8;i++)
    printf("rxbuf[%d]:%d\r\n",i,CAN2_Handler.pRxMsg->Data[i]);
}
#endif	
...
{	
    u16 i=0;
    CAN2_Handler.pTxMsg->StdId=0XAA;        //标准标识符
    CAN2_Handler.pTxMsg->ExtId=0xAA;        //扩展标识符(29位)
    CAN2_Handler.pTxMsg->IDE=CAN_ID_STD;    //使用标准帧
    CAN2_Handler.pTxMsg->RTR=CAN_RTR_DATA;  //数据帧
    CAN2_Handler.pTxMsg->DLC=len;                
    for(i=0;i<len;i++)
    CAN2_Handler.pTxMsg->Data[i]=msg[i];
...
{		   		   
 	u32 i;
    if(HAL_CAN_Receive(&CAN2_Handler,CAN_FIFO0,0)!=HAL_OK) return 0;//接收数据	
    for(i=0;i<CAN2_Handler.pRxMsg->DLC;i++)
    buf[i]=CAN2_Handler.pRxMsg->Data[i];
